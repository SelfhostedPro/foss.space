...
function HomePage() {
  ...
  const threads = [
    ...
    },
    {
      ...
      tags: [{ name: 'guide', slug: 'guide' }, { name: 'beginners', slug: 'beginners' }],
      postCount: 12,
      viewCount: 345,
    ...
    },
    {
      ...
      tags: [{ name: 'javascript', slug: 'javascript' }, { name: 'learning', slug: 'learning' }],
      postCount: 8,
      viewCount: 210,
          ...
          <Card key={thread.id}>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <CardDescription className="mt-1">
                    Posted in <Link to={`/categories/${thread.category.slug}`} className="text-primary hover:underline">{thread.category.name}</Link> by{' '}
                    <Link to={`/users/${thread.author.username}`} className="text-primary hover:underline">{thread.author.name}</Link>
                  </CardDescription>
                </div>
            ...
            <CardContent className="pb-2">
              <div className="flex flex-wrap gap-1">
                {thread.tags.map((tag) => (
                  <Badge key={tag.slug} variant="secondary" className="px-2 py-0.5 text-xs">
                    {tag.name}
                  </Badge>
                ))}
              </div>
